...
- 🤖 **Smart AI Recommendations:** Automatic video creation based on user preferences, market trends, and past interactions.
...
InstaAD is built on a modern, decoupled Client-Server architecture, utilizing containerization and continuous integration for seamless deployment.
...
* **CI/CD Pipeline:** GitHub Actions automatically tests and builds the backend code upon every push, publishing the latest Docker Image directly to Docker Hub.

## 🛠 Tech Stack
* **Frontend GUI:** Python, PyQt5, QSS (Styling)
* **Backend API:** Python, FastAPI, Uvicorn
* **Database:** MongoDB, PyMongo
* **AI / ML Models:** KIE.ai + Seedance 1.5 Pro (Text-to-Video generation model)
...
Welcome to the InstaAD project! This document provides step-by-step instructions on how to initialize, configure, and run the application on your local machine.
...
> ⚠️ **IMPORTANT INSTRUCTION:** Inside the ZIP archive where you found this guide (or provided separately by the team), there is a file named `.env`. You must extract this `.env` file and place it directly into the root directory of the cloned project (`InstaAD_generator/`). The system will not operate without this file in the correct location.
...
3. Run the application (You can also run this directly from your IDE):
...
This method isolates the backend services and the database using containerization. Thanks to our CI/CD pipeline, you do not need to build the backend locally. The latest stable version is always pulled directly from the cloud.
